...
def objective(problem: str, config: dict):
    ...
    values = [config.get(f"x{i}", 0.0) for i in range(6)]
    x = torch.tensor(values, dtype=torch.float).reshape(1, -1)
    evaluation = {"F1": round(float(Hartmann(dim=6).evaluate_true(x)), 3)}
    report(**evaluation)
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        ...
        if current_arg.startswith('--') and i + 1 < len(sys.argv) and not sys.argv[i + 1].startswith('--'):
    ...
    with open("hartmann6.txt", "a") as file:
        file.write(f"args {args}\n")
        file.write(f"Original command: {' '.join(sys.argv)}\n")
        file.write(f"Modified command: {' '.join(modified_args)}\n")
    objective(problem="hartmann6", config=vars(args))