...
def objective(problem: str, config: dict):
    ...
    values = [config.get(f"x{i}", 0.0) for i in range(10)]
    x = torch.tensor(values, dtype=torch.float).reshape(1, -1)
    evaluation = {"F1": round(float(Levy(dim=10).evaluate_true(x)), 3)}
    report(**evaluation)
